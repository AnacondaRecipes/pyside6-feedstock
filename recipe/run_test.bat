...
if not exist %LIBRARY_LIB%\\PySide6\\include\\pyside.h (echo "FATAL: Failed to find %LIBRARY_INC%\\PySide6\\pyside.h" && exit 6)
if not exist %LIBRARY_LIB%\\shiboken6\\include\\shiboken.h (echo "FATAL: Failed to find %LIBRARY_INC%\\shiboken6\\shiboken.h" && exit 7)
